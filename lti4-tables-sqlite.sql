...
DROP TABLE lti2_consumer;
CREATE TABLE lti2_consumer (
  consumer_pk int(11) PRIMARY KEY,
...
CREATE UNIQUE INDEX lti2_consumer_consumer_key_UNIQUE ON lti2_consumer (consumer_key ASC);
CREATE UNIQUE INDEX lti2_consumer_platform_UNIQUE ON lti2_consumer (platform_id ASC, client_id ASC, deployment_id ASC);
...
CREATE TABLE lti2_access_token (
  ...
  CONSTRAINT lti2_access_token_lti2_consumer_FK1 FOREIGN KEY (consumer_pk) REFERENCES lti2_consumer (consumer_pk)
) ;
...
DROP TABLE lti2_context;
CREATE TABLE lti2_context (
  context_pk int(11) NOT NULL PRIMARY KEY,
  consumer_pk int(11) NOT NULL,
...
CREATE INDEX lti2_context_consumer_id_IDX ON lti2_context (consumer_pk ASC);
...
DROP TABLE lti2_resource_link;
CREATE TABLE lti2_resource_link (
  resource_link_pk int(11) PRIMARY KEY,
  ...
  CONSTRAINT lti2_resource_link_lti2_consumer_FK1 FOREIGN KEY (consumer_pk) REFERENCES lti2_consumer (consumer_pk),
...
DROP TABLE lti2_user_result;
CREATE TABLE lti2_user_result (
  user_result_pk int(11) PRIMARY KEY,
